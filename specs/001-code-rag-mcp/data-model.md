...
| `context_path` | str | Hierarchical location | Required, e.g., "File: auth.py > Class: UserAuth > Method: login" |
...
| `node_type` | str | AST node type | Required, e.g., "function_definition", "class_definition" |
...
- `context_path` is constructed during AST traversal: parent contexts prepended to current node
- `token_count` is computed using tiktoken library before storage
- Chunks with `token_count > 2048` (default limit) are split further during AST traversal
...
Represents the vector embedding of a CodeChunk, generated by Jina Embeddings v2.
...
| `vector` | float[] | 768-dimensional embedding | Required, length = 768 |
| `model_version` | str | Embedding model identifier | Required, e.g., "jina-embeddings-v2-base-code" |
...
- `vector` is generated by calling Jina Embeddings API with `source_code` + `context_path` as input
- `model_version` is stored to detect when re-embedding is needed (e.g., model upgrade)
...
schema = {
    ...
    "start_line": int,
    "end_line": int,
    "vector": vector(768),        # 768-dimensional float array
    "model_version": str,
...
| `jina_api_key` | str | API key for embeddings | Required (env var) |
